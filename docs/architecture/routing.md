...
|---|---|---|---|
| 2026-05-19 | Static content | `/how-it-works` rebuilt as a scroll-driven "Bubble Chamber": a `?role=`-persisted buyer/teacher journey toggle (ARIA tablist), per-step full-height sections with an ambient bubble field + parallax numeral + inflating Polaroid bubble. Same i18n keys/content; no routing or data change. Components under `components/how-it-works/*`. | Presentational redesign; logged here only so the route's shape (a single page with a journey query param, not two routes) is documented for contributors. |
...
| Route | Purpose | Notes |
|---|---|---|
| `/[locale]/how-it-works` | How it works | "Bubble Chamber" scroll experience: buyer/teacher journey toggle persisted in `?role=` (single page, not two routes), per-step sections, then static glossary (Lightning / Wapu / Nostr logos) + no-custody pitch + dual CTA. Renamed from `/como-funciona` (ADR 0023, clean pre-launch rename — no redirect). |
